...
def test_robust_avar_nested_upward_corrects_relative_to_naive():
    """The nested-bootstrap estimator must be >= the naive estimate on
    average: it adds a non-negative bias correction for the optimizer's
    curse, so its mean should exceed the naive mean across a small Monte
    Carlo.  (It need not exceed it for every single trial.)"""
    from convexrisk.mono_period import avar_efficient_portfolio
    from convexrisk.risk_measures import empirical_avar

    rng = np.random.default_rng(77)
    true_mu    = np.array([0.08, 0.05])
    true_sigma = np.array([[0.05, 0.01], [0.01, 0.03]])
    n, level, target_return, x0 = 50, 0.1, 0.03, 1.0
    n_trials, n_bootstrap = 60, 50  # small counts for speed

    naive_vals, nested_vals = [], []
    for _ in range(n_trials):
        sample = rng.multivariate_normal(true_mu, true_sigma, size=n)
        probs  = np.full(n, 1.0 / n)
        res    = avar_efficient_portfolio(sample, probs, level=level,
                                          target_return=target_return, x0=x0)
        if not res["success"]:
            continue
        pi_hat = res["pi"]
        pnl    = x0 * (sample @ pi_hat)
        naive_vals.append(empirical_avar(pnl, level))
        nested_vals.append(
            robust_avar_nested.__wrapped__(pnl, level, n_bootstrap, sample,
                                           target_return, rng, x0)
            if hasattr(robust_avar_nested, "__wrapped__")
            else __import__(
                "convexrisk.robust", fromlist=["robust_avar_nested"]
            ).robust_avar_nested(pnl, level, n_bootstrap, sample, target_return, rng, x0)
        )
    assert np.mean(nested_vals) >= np.mean(naive_vals)


def test_robust_avar_nested_reduces_mean_absolute_bias():
    """The nested-bootstrap estimator should achieve strictly lower mean
    absolute bias than the naive estimator in the Gaussian experiment
    (Section 6.5), which is the main claimed improvement over the sup.
    Tested with a smaller n_trials (150) and n_bootstrap (50) for speed;
    the effect is robust across seeds."""
    ...
    result = optimizer_curse_experiment(
        ...
        level=0.1,
        target_return=0.03,
        n_bootstrap=50,   # reduced for speed; effect is large enough to survive
        n_trials=150,
        rng=rng,
        quantile_confidence=0.8,
        include_nested=True,
    )
    assert result["nested_mean_abs_bias"] <= result["naive_mean_abs_bias"], (
        f"nested |bias| {result['nested_mean_abs_bias']:.4f} should be "
        f"<= naive |bias| {result['naive_mean_abs_bias']:.4f}"
    )
    # The sup should still overcorrect (sanity: this is the known failure mode)
    assert result["sup_mean_abs_bias"] >= result["naive_mean_abs_bias"], (
        "sup |bias| should still exceed naive |bias| (known overcorrection)"
    )


def test_nested_bootstrap_coherence_cash_invariance():
    """rho_nested(X + m) == rho_nested(X) - m (cash invariance).

    Since rho_nested(X) = AVaR_n(X) + bias_hat where bias_hat does not
    depend on X, cash invariance of AVaR_n implies cash invariance of
    rho_nested.  We verify this numerically.
    """
    from convexrisk.robust import robust_avar_nested
    import numpy as np

    rng = np.random.default_rng(55)
    sample = rng.multivariate_normal([0.08, 0.05],
                                      [[0.05, 0.01],[0.01,0.03]], size=50)
    pnl = sample @ np.array([0.6, 0.4])
    n_bootstrap, level, target_return = 30, 0.1, 0.03

    rng1 = np.random.default_rng(99)
    rng2 = np.random.default_rng(99)  # same seed -> identical draws
    val_X   = robust_avar_nested(pnl,       level, n_bootstrap, sample, target_return, rng1)
    val_Xm  = robust_avar_nested(pnl + 0.5, level, n_bootstrap, sample, target_return, rng2)
    assert abs(val_Xm - (val_X - 0.5)) < 1e-8, (
        f"Cash invariance violated: rho(X+0.5)={val_Xm:.6f}, "
        f"rho(X)-0.5={val_X-0.5:.6f}"
    )
